...
import { Search, Download, Trash2, Filter, Clock, User, AlertCircle, MessageCircle } from 'lucide-react';
...
export default function HideItemDatabaseView() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to unhide this item?")) return;
    
    try {
      const res = await fetchWithAuth(`${API_URL}/hidden-items/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) fetchData();
    } catch (err: any) {
