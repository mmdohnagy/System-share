...
import { Search, Check, X, Clock, User, AlertCircle, ChevronDown, Filter, Trash2, Package, MapPin, Shield, Send, Sparkles, Globe, MessageCircle } from 'lucide-react';
...
interface Product { id: number; brand_id: number; brand_name: string; product_name: string; is_offline?: boolean; }
...
export default function HideItemView() {
    ...
    try {
      ...
      if (branchesRes.ok) {
        ...
        if (Array.isArray(allBranches)) {
          setBranches(allBranches.filter((b: Branch) => b.brand_id === Number(selectedBrand)));
        } else {
          setBranches([]);
        }
      }
  ...
  const handleSelectAll = () => {
    const filteredIds = products
      .filter(p => p.product_name.toLowerCase().includes(searchQuery.toLowerCase()) && !p.is_offline)
      .map(p => p.id);
