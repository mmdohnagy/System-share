...
export default function TechnicalBackOfficeView() {
  ...
  if (!user || !['Technical Back Office', 'Manager', 'Super Visor', 'Restaurants'].includes(user.role_name)) {
    return (
  ...
  const [activeTab, setActiveTab] = useState<'busy' | 'database' | 'technical' | 'requests'>('busy');
    ...
    if (activeTab === 'busy') {
      const fetchConfig = async () => {
        try {
          ...
          setBrands(Array.isArray(brData) ? brData : []);
          setBranches(Array.isArray(bData) ? bData : []);
          setReasons(Array.isArray(rData) ? rData : []);
          setResponsibleParties(Array.isArray(respData) ? respData : []);
        } catch (err: any) {
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex flex-col gap-6 overflow-y-auto pr-2">
      ...
      {activeTab === 'technical' ? (
        ...
        <div className="flex-1 p-10 lg:p-12 bg-white dark:bg-zinc-900 relative overflow-y-auto">
